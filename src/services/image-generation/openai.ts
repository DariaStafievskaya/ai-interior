...
import { createInteriorPrompt } from "./prompt";
...
export async function generateInterior(
  ...
  try {
    const requestBody = await createImageEditRequestBody(imagePath, generationContext);
...
async function createImageEditRequestBody(
  imagePath: string,
  generationContext: GenerationContext
): Promise<FormData> {
  ...
  formData.append("prompt", createInteriorPrompt(generationContext));
